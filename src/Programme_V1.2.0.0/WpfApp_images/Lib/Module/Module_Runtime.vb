...
Public Module Module_Runtime
    Public Sub Runtime(Window As MainWindow)
        ...
        Dim Liste_Final As New List(Of String)

        For Each ImagePath In ListeFiles
            If Window.RadioButton_Multi_X1.IsChecked Then

                Liste_Final.Add(ImagePath)
            ElseIf Window.RadioButton_Multi_X4.IsChecked Then

                Liste_Final.Add(ImagePath)
                Liste_Final.Add(ImagePath)

            ElseIf Window.RadioButton_Multi_X16.IsChecked Then
                ...
                Liste_Final.Add(ImagePath)
                Liste_Final.Add(ImagePath)
            ElseIf Window.RadioButton_Multi_X64.IsChecked Then
        ...
        Dim LenH = CSng(PasH) + CSng(2.0)
        Dim LenW = CSng(PasW) + CSng(2.0)
                    ...
                    Using image = New MagickImage(ListeFiles.Item(NoPicture Mod (ListeFiles.Count)))
                        ...
                        For h As Integer = H1 To H2
                            For w As Integer = W1 To W2
                                ...
                                Dim Pixel = Img.GetPixel(w, h)
                                Dim Coef = Module_TuileSignals.Tuiles(CH, CW, h, w, CInt(PasH) + 2, 2 + CInt(PasW), Window)
                                Dim oldPixeRH = Class_ArrayByte.GetValue(h, w)
                                oldPixeRH.Add(Coef, Pixel)
                                Class_ArrayByte.SetValue(h, w, oldPixeRH)
                            Next
                    ...
                    Using image = New MagickImage(Liste.Item(NoPicture))
                        ...
                        For h As Integer = H1 To H2
                            For w As Integer = W1 To W2
                                ...
                                If h >= image.Height Then Continue For
                                Dim Pixel = Img.GetPixel(w, h)
                                Coef = Module_TuileSignals.Tuiles(CH, CW, h, w, CInt(PasH) + 2, CInt(PasW) + 2, Window)
